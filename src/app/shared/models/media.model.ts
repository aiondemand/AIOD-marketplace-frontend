...
export class Media {
    ...
    private isImage(format: string): boolean {
        return format.includes('JPEG') || format.includes('PNG') ||
        format.includes('GIF') || format.includes('BMP') ||
        format.includes('TIFF') || format.includes('WebP') ||
        format.includes('SVG');
    }
    private isVideo(format: string): boolean {
        return format.includes('MP4') || format.includes('AVI') ||
        format.includes('MKV') || format.includes('MOV') ||
        format.includes('WMV') || format.includes('FLV') ||
        format.includes('MPEG');
    }
    private isAudio(format: string): boolean {
        return format.includes('MP3') || format.includes('WAV') ||
        format.includes('AAC') || format.includes('FLAC') ||
        format.includes('OGG') || format.includes('WMA') ||
